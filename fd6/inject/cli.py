...
Run with: `python -m fd6.inject <subcommand>`.
...
Workflow (typical):
  ...
  2. python -m fd6.inject status
  3. python -m fd6.inject scan-float 100.0          # known coord, e.g., a sphere's X
  4. (move sphere in-game to a different X)
  5. python -m fd6.inject narrow 200.0
  6. (repeat narrow until ~1 result)
  7. python -m fd6.inject dump <addr> 256           # identify struct fields
  8. python -m fd6.inject find-refs <struct_addr>   # get stable code-side AOBs
  9. python -m fd6.inject save-pattern shape_array_ref "<AOB>" --offset 3
  10. (edit fh6_patterns.json to fill in shape_struct.stride_bytes + fields manually based on dump)
  11. python -m fd6.inject test-injector
"""
...
from fd6.inject.win_process import find_process_id
...
def cmd_test_injector(args) -> int:
    from fd6.inject.fh6_injector import FH6Injector
    pid = _resolve_pid(args.pid)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m fd6.inject", description=__doc__,
