...
Workflow (matching forza-painter's documented technique):
  ...
  2. Run `python -m fd6.inject find-pid` to confirm forzahorizon6.exe is detected.
  3. Run `python -m fd6.inject scan-float 1234.5` (or whatever coord) — saves matches to a scan file.
  4. User moves the sphere in-game. Run `python -m fd6.inject narrow 5678.9` — keeps only addresses
...
from fd6.inject.win_process import (
